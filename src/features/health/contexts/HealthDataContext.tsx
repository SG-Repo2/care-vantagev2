...
import { HealthMetrics, HealthError, HealthState } from '../providers/types';
import { HealthMetricsService, HealthDataSource } from '../services/HealthMetricsService';
...
interface ProviderMetrics extends Partial<HealthMetrics> {
  score?: number | null;
...
const healthMetricsService: HealthMetricsService = {
  ...
  getProviderData: async (source: HealthDataSource) => {
...
type HealthDataAction =
  ...
  | { type: 'UPDATE_WEEKLY_DATA'; payload: WeeklyData };
...
export const HealthDataProvider: React.FC<HealthDataProviderProps> = ({
  ...
  const initializeHealthProviders = useCallback(async () => {
    try {
      ...
      for (const provider of providers) {
        ...
        if (!isMountedRef.current) return;
        dispatch({ type: 'UPDATE_PROVIDER_DATA', payload: { provider, data: { ...data, score: data.daily_score ?? 0 } }});
      }
  ...
  const refresh = useCallback(async () => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
      await initializeHealthProviders();
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false });
    }
